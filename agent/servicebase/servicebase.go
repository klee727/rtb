...
/*
func main() {
	sb := NewServiceBase([]string{"127.0.0.1:2181"}, "rtb-test", "mtl")
	sb.NewWatcher("rtbRequestRouter", "agents").OnNew(func(name string, uri string) {
		fmt.Println("new router service found:", name, uri)
	}).Watch()

	sb.NewWatcher("rtbAgentConfiguration", "agents").OnNew(func(name string, uri string) {
		fmt.Println("new agent configure service found:", name, uri)
	}).Watch()
